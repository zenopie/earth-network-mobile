...
/**
 * Caretaker Fund — the one-human-one-vote allocation vote: x/allocation's human
 * stream, backed by
 * x/caretaker and open to registered humans. "Actual" shows how the vote is
 * currently split across options (from each option's amount_allocated); "Preferred"
 * shows the signed-in human's own split with a button to update it.
 */
class CaretakerFundFragment : Fragment() {
    ...
    // id -> description, populated from the chain's allocation options.
    private val optionNames = mutableMapOf<Long, String>()
    // Actual + preferred as (optionId, percent) pairs.
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        return inflater.inflate(R.layout.fragment_caretaker_fund, container, false)
    }
    ...
    private fun loadActualAllocations() {
        lifecycleScope.launch {
            try {
                val options = withContext(Dispatchers.IO) { Allocation.allocationOptions(StreamId.STREAM_ID_HUMAN) }
        ...
        lifecycleScope.launch {
            try {
                ...
                val (options, voter) = withContext(Dispatchers.IO) {
                    val opts = Allocation.allocationOptions(StreamId.STREAM_ID_HUMAN)
                    val v = if (address.isNullOrEmpty()) emptyList() else Allocation.voterAllocations(StreamId.STREAM_ID_HUMAN, address)
                    opts to v
                }
                if (!isAdded) return@launch
                optionNames.clear()
    ...
    private fun openSetAllocation() {
        val fragment = SetAllocationFragment.newInstance(
            SetAllocationFragment.FUND_TYPE_CARETAKER, "Caretaker Fund"
        )
